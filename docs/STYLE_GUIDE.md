...
| `--kindyr-surface` | `#0a2559` | cards, panels, modals |
| `--kindyr-surface-2` | `#12408a` | botones secundarios, badges, hovers/active |
| `--kindyr-inset` | `#071c45` | inputs, chips, listas, zonas hundidas sobre surface |
...
| Scrollbar | thumb `accent` 14px, borde 2px ink, radio 0; track `inset` con borde ink; hover accent aclarado |

**Temas `data-theme`:** `midnight` (default), `navy` `#010718`, `azure` `#08245b`, `forest` `#04160e`, `crimson` `#1c0509`, `ember` `#190c03`, `grape` `#120826`, `steel` `#dceaff` (light, `color-scheme:light`). Todos usan mismos tokens, solo cambian bg/surface. Legacy `light/green/kindyr/neobrutal` siguen en `styles.css` pero no se usan.
...
## 8. i18n
`common.js:7` `I18N = {es:{}, en:{}}` `t(key,vars)` `split('{var}').join` fallback `es`. `translateElement` actualiza `[data-i18n]`, `[data-i18n-placeholder]` etc. Temas: `data-theme="midnight|navy|azure|forest|crimson|ember|grape|steel"`.

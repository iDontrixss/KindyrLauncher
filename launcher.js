...
async function launchGame() {
  ...
  try {
    ...
    const launchInstance = (typeof launcherInstances !== 'undefined' && Array.isArray(launcherInstances))
      ...
      : null
    const effMinRam = launchInstance && Number(launchInstance.memoryMinMb) > 0
      ? String(Math.round(Number(launchInstance.memoryMinMb))) + 'M'
      : settings.minRam
    const effMaxRam = launchInstance && Number(launchInstance.memoryMaxMb) > 0
      ? String(Math.round(Number(launchInstance.memoryMaxMb))) + 'M'
      : settings.maxRam
    const effArgs = launchInstance && String(launchInstance.javaArgs || '').trim()
      ? String(launchInstance.javaArgs)
      : settings.javaArgs
    const result = await window.kindyrAPI.launcher.launch({
      ...
      maxRam: effMaxRam,
      customArgs: effArgs,
      javaHome: (launchInstance && launchInstance.javaHome) || ''
    })
    if (!result.ok) {
      if (result.cancelled) {

...
## Build — pnpm overrides moved to `pnpm-workspace.yaml`

- pnpm 11 no longer reads the `pnpm.overrides` field from `package.json` (warned as ignored),
  so `js-yaml ^4.3.2` and `undici ^7.28.0` are now declared under `overrides:` in
  `pnpm-workspace.yaml` and the obsolete `pnpm` block was removed from `package.json`.
...
- Full suite green (137 tests), `node scripts/check-syntax.js` clean.

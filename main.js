...
const { app, BrowserWindow, ipcMain, shell, nativeImage, safeStorage, dialog } = require('electron')
const fs = require('fs')
...
// Forzar scrollbars clasicas: con overlay scrollbars (Windows 11 con
// "mostrar siempre las barras" desactivado) Chromium dibuja un thumb
// flotante redondeado e ignora el estilo ::-webkit-scrollbar.
// OverlayScrollbar = overlay generico, FluentScrollbar = estilo Fluent de Win11.
try {
...
async function persistMicrosoftCredentials(account, xboxManager, token) {
  ...
  const clientId = getMicrosoftAuth().token?.client_id || ''
  const updated = await withMicrosoftAccounts(async (accounts) => {
    ...
    const updatedAccount = {
      ...
      client_id: clientId || accounts[index].client_id || '',
  ...
  return updated.find(item => item.id === credentials.uuid)
}
...
function loadCustomInstances() {
  ...
  try {
    ...
    if (migrateInstanceIcons(customInstancesCache)) {
      try {
        fs.writeFileSync(file, JSON.stringify(customInstancesCache, null, 2))
      } catch {}
    }
...
// Repara iconos guardados con ruta relativa (versiones previas guardaban
// solo el nombre): los resuelve contra la carpeta de la instancia y
// descarta los que ya no existen para no mostrar imagenes rotas.
function migrateInstanceIcons(instances) {
  if (!Array.isArray(instances)) return false
  let changed = false
  for (const instance of instances) {
    const icon = instance && instance.icon
    if (!icon || typeof icon !== 'object' || !icon.file || path.isAbsolute(icon.file)) continue
    const candidate = path.join(getInstanceDir(instance.id), path.basename(icon.file))
...
async function installCurseForgeProject(payload = {}) {
  ...
  if (projectType === 'modpack') {
    ...
    return { type: 'instance', path: installed.path, instance: installed.instance, version: file, file }
  }
...
async function getCurseForgeSingleFile(modId, fileId) {
  const apiKey = getCurseForgeApiKey()
  if (!apiKey) throw new Error('CurseForge no configurado.')
  const url = CURSEFORGE_API + '/mods/' + encodeURIComponent(String(modId)) + '/files/' + encodeURIComponent(String(fileId))
  const res = await fetch(url, { headers: { 'x-api-key': apiKey, Accept: 'application/json', 'User-Agent': MODRINTH_USER_AGENT } })
  const body = await res.json().catch(() => ({}))
  if (!res.ok) throw new Error(body.error || body.message || 'CurseForge no respondió bien.')
...
async function downloadCurseForgeModpackEntry(entry, modsDir) {
...
async function installCurseForgeModpackInstance(file, project) {
  ...
  try {
    ...
    let { loader, loaderVersion } = manifest
    if (loader !== 'vanilla' && !loaderVersion) {
      const supported = await getSupportedMinecraftVersions(loader)
      loaderVersion = pickLatestLoaderVersion(loader, manifest.minecraftVersion, supported)
    }
    if (loader !== 'vanilla' && !loaderVersion) {
    ...
    await runWithConcurrency(manifest.files, maxConcurrent, async (entry) => {
      try {
        const ok = await downloadCurseForgeModpackEntry(entry, modsDir)
        if (!ok) failedDownloads.push(`project ${entry.projectID} file ${entry.fileID}`)
      } catch (error) {
        failedDownloads.push(`project ${entry.projectID} file ${entry.fileID} (${error.message || error})`)
      }
    })
    await extractZipEntries(zipPath, minecraftRoot, {
      maxEntryBytes: 512 * 1024 * 1024,
      maxTotalBytes: 2 * 1024 * 1024 * 1024,
      mapEntry: (normalized) => {
        const relative = mapCurseForgeOverrideEntry(normalized, manifest.overrides)
        return relative && isSafeRelativePath(relative) ? relative : null
      }
    })
    if (failedDownloads.length) {
      throw new Error(`No se pudieron descargar ${failedDownloads.length} archivo(s) del modpack.`)
    }
    const instance = {
      id: instanceId,
      name: manifest.name || project.title || 'Modpack',
...
async function getSupportedMinecraftVersions(loader) {
  ...
  const request = (async () => {
    ...
    if (key === 'vanilla') {
      ...
      const latestLoaders = sortLoaderVersionsAscending(normalizeLoaderListEntries(loaders))
      const latestStableLoader = [...latestLoaders].reverse().find(item => item.stable)
      const latestLoader = latestStableLoader?.version || latestLoaders[latestLoaders.length - 1]?.version || ''
...
function normalizeLoaderListEntries(raw) {
  const arr = Array.isArray(raw) ? raw : []
  const out = []
  for (const item of arr) {
    const node = item && typeof item === 'object'
      ? (item.loader && typeof item.loader === 'object' ? item.loader : item)
      : null
    const version = String(node?.version || '').trim()
    if (!version) continue
    out.push({ version, stable: node.stable === true })
  }
  return out
}

function sortLoaderVersionsAscending(list) {
  const arr = Array.isArray(list) ? [...list] : []
  let semver = null
  try { semver = getSemver() } catch {}
  const rank = (entry) => {
    if (semver) {
      const valid = semver.valid(String(entry.version || '').trim(), { loose: true })
      if (valid) return 'v:' + valid
    }
    return 's:' + String(entry.version || '')
  }
  const keyOf = (entry) => rank(entry)
  arr.sort((a, b) => {
    const ka = keyOf(a)
    const kb = keyOf(b)
    if (ka[0] === 'v' && kb[0] === 'v') {
      try { return semver.compare(ka.slice(2), kb.slice(2)) } catch {}
    }
...
// Lista versiones del loader para un Minecraft dado (canal Estable/Mas reciente).
// Devuelve [{ version, stable }] en orden ascendente.
async function listLoaderVersions(loader, minecraftVersion) {
  const name = String(loader || '').trim()
  const mc = String(minecraftVersion || '').trim()
  if (!['fabric', 'forge', 'neoforge', 'quilt'].includes(name)) return []
  if (!mc) return []
  if (name === 'fabric' || name === 'quilt') {
    const baseUrl = name === 'fabric' ? FABRIC_META : QUILT_META
    const cachePrefix = name === 'fabric' ? 'fabric' : 'quilt'
    const [games, loaders] = await Promise.all([
      fetchJsonCached(baseUrl + '/versions/game', { cacheKey: cachePrefix + '-game', ttlMs: VERSION_CACHE_TTL_MS }),
      fetchJsonCached(baseUrl + '/versions/loader/' + encodeURIComponent(mc), { cacheKey: cachePrefix + '-loader-' + mc, ttlMs: VERSION_CACHE_TTL_MS }).catch(() => (
        fetchJsonCached(baseUrl + '/versions/loader', { cacheKey: cachePrefix + '-loader', ttlMs: VERSION_CACHE_TTL_MS })
      ))
    ])
    const gameOk = Array.isArray(games)
      ? games.some(game => String(game?.version || game?.id || game || '').trim() === mc)
      : true
    if (!gameOk) return []
    const sorted = sortLoaderVersionsAscending(normalizeLoaderListEntries(loaders))
    if (!sorted.length) return []
    if (!sorted.some(item => item.stable)) sorted[sorted.length - 1].stable = true
    return sorted
  }
  const supported = await getSupportedMinecraftVersions(name)
  let raws = []
  if (name === 'forge') {
    raws = supported
      .filter(item => item.minecraft === mc && item.loaderVersion)
      .map(item => ({ version: item.loaderVersion, stable: false, raw: item.raw }))
  } else {
    const prefix = getNeoForgePrefix(mc)
    raws = supported
      .filter(item => item.minecraftPrefix === prefix && item.loaderVersion)
      .map(item => ({ version: item.loaderVersion, stable: false, raw: item.raw }))
  }
  const sorted = sortLoaderVersionsAscending(raws)
  if (!sorted.length) return []
  sorted[sorted.length - 1].stable = true
  return sorted
}
...
function createLauncherInstance(payload = {}) {
  ...
  const defaultName = loader === 'vanilla'
    ? 'Minecraft ' + version
    : loader[0].toUpperCase() + loader.slice(1) + ' ' + version
  const name = String(payload.name || '').replace(/[\x00-\x1F\x7F]/g, '').trim().slice(0, 48) || defaultName
  const icon = resolveInstanceIcon(payload.icon)
  const instance = {
    id: instanceId,
    name,
    version,
  ...
  if (icon && (icon.file || icon.fa)) {
    instance.icon = icon.file
      ? { file: path.join(getInstanceDir(instanceId), icon.file) }
      : { fa: icon.fa, bg: icon.bg }
  }
  registerCustomInstance(instance)
  ensureInstanceFolders(instance)
  if (icon && icon.file && icon.sourcePath) {
    try {
      fs.copyFileSync(icon.sourcePath, path.join(getInstanceDir(instance.id), icon.file))
    } catch (error) {
      throw new Error('No se pudo guardar el ícono: ' + (error.message || String(error)))
    }
  }
...
const INSTANCE_ICON_IMAGE_EXTS = new Set(['.png', '.jpg', '.jpeg', '.webp', '.gif'])
const MAX_INSTANCE_ICON_BYTES = 5 * 1024 * 1024

// Normaliza el icono elegido en el asistente. Acepta preset {fa,bg} o
// subida {path}. Para subidas devuelve {file, sourcePath} para copiar
// al crear la instancia (el renderer no escribe archivos).
function resolveInstanceIcon(iconPayload) {
  if (!iconPayload || typeof iconPayload !== 'object') return null
  const fa = String(iconPayload.fa || '').trim()
  const bg = String(iconPayload.bg || '').trim()
  if (fa || bg) {
    if (!/^[a-z0-9-]+$/i.test(fa)) throw new Error('Ícono inválido.')
    if (!/^#[0-9a-f]{6}$/i.test(bg)) throw new Error('Color de ícono inválido.')
    return { fa, bg }
  }
  const iconPath = String(iconPayload.path || '').trim()
  if (!iconPath) return null
  const ext = path.extname(iconPath).toLowerCase()
  if (!INSTANCE_ICON_IMAGE_EXTS.has(ext)) throw new Error('El ícono debe ser PNG, JPG, WebP o GIF.')
  let stat = null
  try {
    stat = fs.statSync(iconPath)
    if (!stat.isFile()) throw new Error('No es un archivo válido.')
  } catch (error) {
    throw new Error('No se pudo leer el ícono: ' + (error.message || String(error)))
  }
  if (stat.size > MAX_INSTANCE_ICON_BYTES) throw new Error('El ícono supera los 5 MiB.')
  return { file: 'icon' + ext, sourcePath: iconPath }
}
...
async function downloadToFile(url, destination, opts = {}) {
  ...
  if (parsed.protocol !== 'https:') throw new Error('URL de descarga invalida (solo HTTPS).')
  const externalSignal = opts && opts.signal ? opts.signal : null
  if (externalSignal && externalSignal.aborted) {
    const reason = externalSignal.reason
    throw reason instanceof Error ? reason : new Error('Descarga cancelada.')
  }
  const timeoutMs = Number(opts && opts.timeoutMs) > 0 ? Math.floor(Number(opts.timeoutMs)) : 60000
  const controller = new AbortController()
  const forwardAbort = () => {
    try { controller.abort(externalSignal.reason) } catch {}
  }
  if (externalSignal) {
    try { externalSignal.addEventListener('abort', forwardAbort, { once: true }) } catch {}
  }
  const timer = setTimeout(() => {
    try { controller.abort(new Error(`Descarga sin respuesta por ${Math.round(timeoutMs / 1000)}s: ${parsed.host}`)) } catch {}
  }, timeoutMs)
  if (timer && typeof timer.unref === 'function') {
    try { timer.unref() } catch {}
  }
  try {
    return await downloadToFileInner(parsed, destination, controller.signal)
  } catch (error) {
...
async function downloadToFileInner(parsed, destination, signal) {
  const throwIfCancelled = () => {
    if (inicioCancelado || xmclCancellationRequested) throw new Error('Descarga cancelada.')
  ...
  throwIfCancelled()
  let response = await fetch(parsed, { headers, signal })
  if (start > 0 && response.status === 416) {
    try { fs.unlinkSync(partPath) } catch {}
    start = 0
    delete headers.Range
    throwIfCancelled()
    response = await fetch(parsed, { headers, signal })
  }
  ...
  try {
    ...
    throwIfCancelled()
    await pipeline(Readable.fromWeb(response.body), fs.createWriteStream(partPath, { flags: writeFlags }), { signal })
    throwIfCancelled()
    try {
...
function assertSafeReleaseDownloadUrl(value) {
  const parsed = new URL(String(value || ''))
  const host = parsed.hostname.toLowerCase()
  const allowedHosts = ['github.com', 'objects.githubusercontent.com']
  const allowed = parsed.protocol === 'https:' && allowedHosts.some(h => host === h || host.endsWith('.' + h))
  if (!allowed) throw new Error('URL de descarga no permitida.')
...
function showUpdateConfirmation(updateInfo) {
  ...
  hardenWindowNavigation(updateConfirmationWindow)
...
async function fetchWithRetry(url, options, label) {
  // Ciclo 5s -> espera 3s -> 5s con logs en tiempo real
  for (let intento = 1; intento <= 2; intento++) {
    const ctrl = new AbortController()
    const timeout = setTimeout(() => ctrl.abort(), 5000)
    console.log(`[Kindyr][${label}] Intento ${intento}/2 — GET ${url} (timeout 5s)`)
    try {
      const res = await fetch(url, { ...options, signal: ctrl.signal })
      clearTimeout(timeout)
      console.log(`[Kindyr][${label}] Respuesta ${res.status} en intento ${intento}`)
      return res
    } catch (e) {
      clearTimeout(timeout)
      const isTimeout = e.name === 'AbortError' || String(e.message).includes('aborted')
      console.warn(`[Kindyr][${label}] ${isTimeout ? 'Timeout 5s' : 'Error'} en intento ${intento}/2: ${e.message || e}`)
      if (intento === 1) {
        console.log(`[Kindyr][${label}] Esperando 3s antes de reintento...`)
        await sleep(3000)
      } else {
        throw e
      }
...
async function fetchUpdateApproval() {
  ...
  console.log(`[Kindyr][update.json] Verificando si el ciclo está autorizado...`)
  const res = await fetchWithRetry(url, { cache: 'no-store', headers: { 'Cache-Control': 'no-cache', Accept: 'application/json', 'User-Agent': 'KindyrLauncher/' + app.getVersion() } }, 'update.json')
  if (!res.ok) throw new Error(`update.json fetch failed: HTTP ${res.status}`)
  const data = await res.json()
  console.log(`[Kindyr][update.json] OK: updatesEnabled=${data.updatesEnabled}, approvedAt=${data.approvedAt}`)
  return data
}

async function hasNewerGitHubRelease() {
  let approval = null
  // Recurso 1: update.json — ¿el ciclo está autorizado?
  try {
    ...
    if (lastSeen && String(approval.approvedAt) === String(lastSeen)) {
      console.log(`[Kindyr] Ciclo ya consumido (approvedAt=${approval.approvedAt} == lastSeen), updater DESACTIVADO hasta próximo pnpm update-kindyr`)
      return false
    }
    console.log(`[Kindyr] Ciclo autorizado (approvedAt=${approval.approvedAt} nuevo), pasando a GitHub Releases...`)
  } catch (e) {
    const isTimeout = String(e.message).includes('aborted') || e.name === 'AbortError'
    if (isTimeout || String(e.message).includes('fetch failed')) {
      console.warn(`[Kindyr] CHECK_FAILED en update.json tras 2 intentos (5s+3s+5s). Guardando ciclo como consumido y DESACTIVANDO updater hasta próximo pnpm update-kindyr.`)
      try { if (approval && approval.approvedAt) saveLastApprovedAt(approval.approvedAt) } catch {}
      // También si no pudimos leer approval, intentamos guardar el approvedAt si lo teníamos, sino no
    } else {
      console.warn('[Kindyr] No se pudo leer update.json, no se chequeará GitHub por seguridad:', e.message || e)
    }
    // En CHECK_FAILED igual se desactiva (consume el ciclo) si teníamos approvedAt
    if (approval && approval.approvedAt) { try { saveLastApprovedAt(approval.approvedAt) } catch {} }
    console.log('[Kindyr] Updater DESACTIVADO (CHECK_FAILED en update.json). Próximo check solo tras pnpm update-kindyr.')
    return false
  }

  // Recurso 2: GitHub Releases API — ¿existe versión mayor? (con su propio ciclo 5+3+5)
  let response
  try {
    response = await fetchWithRetry('https://api.github.com/repos/iDontrixss/KindyrLauncher/releases?per_page=20', {
      headers: { Accept: 'application/vnd.github+json', 'User-Agent': 'KindyrLauncher/' + app.getVersion() }
    }, 'GitHub Releases')
  } catch (e) {
    console.warn(`[Kindyr] CHECK_FAILED en GitHub Releases tras 2 intentos (5s+3s+5s): ${e.message || e}`)
    console.log('[Kindyr] Ciclo terminado con CHECK_FAILED. Guardando lastApprovedAt y DESACTIVANDO updater hasta próximo pnpm update-kindyr.')
    try { saveLastApprovedAt(approval.approvedAt) } catch {}
    return false
  }

  if (!response.ok) {
    console.warn(`[Kindyr] GitHub Releases HTTP ${response.status} — se considera CHECK_FAILED`)
    try { saveLastApprovedAt(approval.approvedAt) } catch {}
    console.log('[Kindyr] Updater DESACTIVADO (CHECK_FAILED).')
    return false
  }

  const releases = await response.json()
  if (!Array.isArray(releases)) {
    console.warn('[Kindyr] GitHub Releases data inválida — CHECK_FAILED')
    try { saveLastApprovedAt(approval.approvedAt) } catch {}
    return false
  }
  ...
  // Fin de ciclo: se consume el approvedAt tanto si hay update como si no
  try { saveLastApprovedAt(approval.approvedAt) } catch {}
  if (hasNewer) {
    console.log(`[Kindyr] UPDATE_AVAILABLE: hay versión mayor que ${currentVersion} — mostrando diálogo y luego DESACTIVANDO updater`)
  } else {
    console.log(`[Kindyr] NO_UPDATE: no hay versión mayor que ${currentVersion} — ciclo terminado, updater DESACTIVADO hasta próximo pnpm update-kindyr`)
  }
  return hasNewer
}
...
app.whenReady().then(() => {
  ...
  try {
    if (discordRPC) {
      ...
      discordRPC.initDiscordRPC(stored.discordEnabled !== false).catch(() => {})
    }
  } catch {}
  if (!isOnboardingDone()) {
...
ipcMain.handle('settings-purge-cache', () => {
  ...
  // Solo caché/runtime (_extract). No usar para instancias: es fallback síncrono acotado.
  const removeDirTolerant = (dirPath) => {
    try {
      ...
      try {
        ...
        const removeRecursiveSafe = (currentPath, depth = 0) => {
          if (depth > 64) return
          let st = null
          try { st = fs.lstatSync(currentPath) } catch { return }
          // No seguir symlinks: unlink directo para no borrar fuera de caché.
          ...
          try {
            if (st.isDirectory()) {
              ...
              for (const entry of entries) {
                removeRecursiveSafe(path.join(currentPath, entry), depth + 1)
              }
              try {
...
ipcMain.handle('settings-pick-background', async () => {
  try {
    return await pickBackgroundFile()
  } catch (error) {
    console.error('[settings-pick-background]', error)
    return { ok: false, error: error?.message || String(error) }
  }
})

async function pickBackgroundFile() {
  let result
  try {
    ...
    throw new Error('No se pudo abrir el selector de archivos: ' + (error?.message || String(error)))
  }
  if (result.canceled || !result.filePaths.length) return { ok: false, cancelled: true }
  ...
  try {
    ...
    return { ok: false, error: 'No se pudo crear la carpeta de datos: ' + (e.message || String(e)) }
  }
...
ipcMain.handle('loader-versions', async (_event, payload = {}) => {
  try {
    const versions = await listLoaderVersions(payload.loader, payload.minecraftVersion)
    return { ok: true, versions }
  } catch (error) {
    return { ok: false, error: error.message || String(error) }
  }
...
ipcMain.handle('browse-instance-icon', async () => {
  const result = await dialog.showOpenDialog(mainWindow, {
    title: 'Elegir ícono de la instalación',
    properties: ['openFile'],
  ...
  const ext = path.extname(iconPath).toLowerCase()
  if (!INSTANCE_ICON_IMAGE_EXTS.has(ext)) {
    return { ok: false, error: 'El ícono debe ser PNG, JPG, WebP o GIF.' }
  }
  let stat = null
  try {
    stat = fs.statSync(iconPath)
    ...
    return { ok: false, error: 'No se pudo leer el archivo: ' + (error.message || String(error)) }
  }
  if (stat.size > MAX_INSTANCE_ICON_BYTES) {
...
function persistCustomInstance(instance) {
  instance.updatedAt = new Date().toISOString()
  registerCustomInstance(instance)
  fs.writeFileSync(path.join(getInstanceDir(instance.id), 'instance.json'), JSON.stringify(instance, null, 2))
...
function getEditableInstance(id) {
  const instance = getInstance(id)
  if (!instance) throw new Error('No existe la instancia seleccionada.')
  if (defaultInstances.some(item => item.id === instance.id)) {
    throw new Error('Las instalaciones base no se pueden modificar.')
  }
  return instance
}

ipcMain.handle('instance-rename', (_event, payload = {}) => {
  try {
    const instance = getEditableInstance(payload.id)
    const name = String(payload.name || '').replace(/[\x00-\x1F\x7F]/g, '').trim().slice(0, 48)
    if (!name) throw new Error('Elegí un nombre válido.')
    instance.name = name
...
ipcMain.handle('instance-duplicate', async (_event, payload = {}) => {
  try {
    const source = getEditableInstance(payload.id)
    const baseId = sanitizeInstanceId(source.id + '-copia') || (source.id + '-copia')
    const existingIds = new Set([...defaultInstances.map(i => i.id), ...loadCustomInstances().map(i => i.id)])
    let instanceId = baseId
    let suffix = 2
    while (existingIds.has(instanceId)) instanceId = `${baseId}-${suffix++}`
    const targetDir = getInstanceDir(instanceId)
    fs.mkdirSync(path.dirname(targetDir), { recursive: true })
    await fs.promises.cp(path.join(getInstanceDir(source.id)), targetDir, { recursive: true, force: true })
    await fs.promises.rm(path.join(targetDir, 'launcher-logs'), { recursive: true, force: true }).catch(() => {})
    const instance = {
      ...JSON.parse(JSON.stringify(source)),
      id: instanceId,
      name: String(source.name || 'Instancia') + ' (copia)',
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString()
    }
    if (instance.icon && instance.icon.file) {
      const copied = path.join(targetDir, path.basename(instance.icon.file))
      instance.icon = fs.existsSync(copied) ? { file: copied } : undefined
      if (!instance.icon) delete instance.icon
    }
    persistCustomInstance(instance)
    return { ok: true, instance, instances: getAllInstances() }
  } catch (error) {
...
ipcMain.handle('instance-delete', async (_event, payload = {}) => {
  try {
    const instance = getEditableInstance(payload.id)
    await fs.promises.rm(getInstanceDir(instance.id), { recursive: true, force: true })
    const remaining = loadCustomInstances().filter(item => item.id !== instance.id)
    saveCustomInstances(remaining)
    invalidateStorageCache()
    return { ok: true, instances: getAllInstances() }
  } catch (error) {
    return { ok: false, error: error.message || String(error) }
  }
})

ipcMain.handle('instance-set-icon', (_event, payload = {}) => {
  try {
    const instance = getEditableInstance(payload.id)
    const icon = resolveInstanceIcon(payload.icon)
    const dir = getInstanceDir(instance.id)
    for (const old of fs.readdirSync(dir)) {
      if (/^icon\.(png|jpg|jpeg|webp|gif)$/i.test(old)) {
    ...
    if (!icon) {
      ...
      fs.copyFileSync(icon.sourcePath, path.join(dir, icon.file))
      instance.icon = { file: path.join(dir, icon.file) }
    } else {
      instance.icon = { fa: icon.fa, bg: icon.bg }
    }
    persistCustomInstance(instance)
    return { ok: true, instance }
  } catch (error) {
...
ipcMain.handle('instance-set-channel', (_event, payload = {}) => {
  try {
    const instance = getEditableInstance(payload.id)
    const channel = String(payload.versionType || '').trim()
    if (!['release', 'beta', 'alpha'].includes(channel)) throw new Error('Canal inválido.')
    instance.versionType = channel
    persistCustomInstance(instance)
    return { ok: true, instance }
  } catch (error) {
    return { ok: false, error: error.message || String(error) }
...
function validateInstanceMemoryMb(value) {
  if (value === null || value === undefined || value === '') return null
  const mb = Math.round(Number(value))
  if (!Number.isFinite(mb) || mb < 512 || mb > 32768) throw new Error('RAM inválida: usá números 512-32768.')
  return mb
}

ipcMain.handle('instance-set-launch-opts', (_event, payload = {}) => {
  try {
    const instance = getEditableInstance(payload.id)
    const memoryMinMb = validateInstanceMemoryMb(payload.memoryMinMb)
    const memoryMaxMb = validateInstanceMemoryMb(payload.memoryMaxMb)
    if (memoryMinMb && memoryMaxMb && memoryMinMb > memoryMaxMb) {
...
ipcMain.handle('instance-change-version', (_event, payload = {}) => {
  try {
    const instance = getEditableInstance(payload.id)
    const version = String(payload.version || '').trim()
    const loader = String(payload.loader || 'vanilla').trim()
    ...
    instance.loaderVersion = loader === 'vanilla' ? '' : loaderVersion
    instance.type = loader === 'vanilla' ? 'vanilla' : instance.type === 'modpack' ? 'modpack' : 'loader'
    delete instance.xmclVersionId
    persistCustomInstance(instance)
    return { ok: true, instance }
  } catch (error) {
    return { ok: false, error: error.message || String(error) }
...
ipcMain.handle('instance-repair', (_event, payload = {}) => {
  try {
    const instance = getEditableInstance(payload.id)
    const ids = new Set()
    if (instance.xmclVersionId) ids.add(String(instance.xmclVersionId))
    try {
      for (const id of getExpectedXmclLoaderVersionIds(instance)) ids.add(id)
    } catch {}
    for (const id of ids) {
      if (!id) continue
      fs.rmSync(path.join(getMinecraftRoot(instance.id), 'versions', id), { recursive: true, force: true })
    }
    delete instance.xmclVersionId
    persistCustomInstance(instance)
    return { ok: true, instance }
...
ipcMain.handle('instance-unlink-modpack', (_event, payload = {}) => {
  try {
    const instance = getEditableInstance(payload.id)
    ...
    if (instance.type === 'modpack') instance.type = instance.loader === 'vanilla' ? 'vanilla' : 'loader'
    persistCustomInstance(instance)
    return { ok: true, instance }
  } catch (error) {
    return { ok: false, error: error.message || String(error) }
...
ipcMain.handle('open-external-url', (_event, url) => {
  try {
    const parsed = new URL(String(url || ''))
    const host = parsed.hostname.toLowerCase()
    const allowed = ['modrinth.com', 'www.modrinth.com', 'curseforge.com', 'www.curseforge.com', 'legacy.curseforge.com', 'github.com', 'www.github.com']
...
function beginXmclLaunch(instance) {
  ...
  try {
    if (discordRPC && loadLauncherSettings().discordEnabled !== false) {
      discordRPC.notifyDiscordLaunchStart(instance.name)
    }
  } catch {}
}
...
function getExpectedXmclLoaderVersionIds(instance) {
  if (instance.loader === 'forge') {
    const mc = String(instance.version || '')
    const { core, versions } = getForgeInstallerCandidates(mc, instance.loaderVersion)
    const ids = [`${mc}-forge-${core}`]
    // IDs reales de instaladores legacy: `<mc>-forge<artefacto>` (ej.
    // `1.8.9-forge1.8.9-11.15.1.2318-1.8.9`, sin guion tras "forge").
    for (const artifact of versions) {
      const legacyId = `${mc}-forge${artifact}`
...
// Forge antiguo (<=1.12, incl. 1.8.x) publica el instalador como
// `<mc>-<forge>-<mc>` (doble sufijo, ej. 1.8.9-11.15.1.2318-1.8.9),
// mientras que el moderno usa `<mc>-<forge>`. El loaderVersion guardado
// puede venir pelado ("11.15.1.2318") o completo, asi que se prueban
// ambas variantes antes de rendirse con 404.
function getForgeInstallerCandidates(minecraftVersion, loaderVersionRaw) {
  const mc = String(minecraftVersion || '').trim()
  const raw = String(loaderVersionRaw || '').trim()
  const ordered = []
  const push = (value) => { if (value && !ordered.includes(value)) ordered.push(value) }
  if (mc && raw.startsWith(mc + '-')) push(raw)
  let core = raw
  if (mc && core.startsWith(mc + '-')) core = core.slice(mc.length + 1)
  if (mc && core.endsWith('-' + mc)) core = core.slice(0, -(mc.length + 1))
  if (!mc || !core) return { core: core || raw, versions: ordered.length ? ordered : (raw ? [raw] : []) }
  const single = `${mc}-${core}`
  const double = `${mc}-${core}-${mc}`
  // Era del doble sufijo: 1.7.x - 1.12.x. Fuera de ahi, el simple primero.
  if (/^1\.(7|8|9|10|11|12)(\.|$)/.test(mc)) { push(double); push(single) } else { push(single); push(double) }
  return { core, versions: ordered }
}

function buildForgeInstallerUrls(artifactVersion) {
  const artifactName = `forge-${artifactVersion}-installer.jar`
  const suffix = `net/minecraftforge/forge/${artifactVersion}/${artifactName}`
  return [
    `https://maven.minecraftforge.net/${suffix}`,
    `https://files.minecraftforge.net/maven/${suffix}`
  ]
}
...
// el `versionInfo` completo y el universal.jar va como libreria Forge.
// Equivale a lo que hace XMCL (installLegacyForgeFromZip) sin ejecutar el jar.
async function installLegacyForgeFromInstaller(installerPath, minecraftRoot, forgeVersion) {
  const profileBuf = await readZipEntryBuffer(installerPath, 'install_profile.json')
  if (!profileBuf) throw new Error('El instalador de Forge no trae install_profile.json.')
  let profile = null
  try {
    profile = JSON.parse(profileBuf.toString('utf8'))
  } catch {
    throw new Error('El install_profile.json del instalador de Forge esta corrupto.')
  }
  const versionJson = profile.versionInfo
  const versionId = String(versionJson?.id || '').trim()
  if (!versionId || !Array.isArray(versionJson.libraries)) {
    throw new Error('El instalador de Forge no trae versionInfo usable (formato legacy desconocido).')
  }
  const inheritsFrom = String(versionJson.inheritsFrom || '').trim()
  if (inheritsFrom) {
    const vanillaJson = path.join(minecraftRoot, 'versions', inheritsFrom, inheritsFrom + '.json')
    if (!fs.existsSync(vanillaJson)) {
      throw new Error(`Falta la base vanilla ${inheritsFrom} para instalar Forge ${forgeVersion}.`)
    }
  }
  const universalName = String(profile.install?.filePath || '').split('/').pop()
  if (!universalName || !/^forge-.*-universal\.jar$/.test(universalName)) {
    throw new Error('El instalador de Forge no declara su universal.jar (formato legacy desconocido).')
  }
  const universalBuf = await readZipEntryBuffer(installerPath, universalName, 128 * 1024 * 1024)
  if (!universalBuf || !universalBuf.length) {
    throw new Error(`El instalador de Forge no trae ${universalName}.`)
  }
  const forgeLib = versionJson.libraries.find(item => {
    const name = String(item?.name || '')
    return name.startsWith('net.minecraftforge:forge:') || name.startsWith('net.minecraftforge:minecraftforge:')
  })
  if (!forgeLib) throw new Error('El versionInfo del instalador no declara la libreria Forge.')
  const libRelative = resolveXmclMavenCoordinate(forgeLib.name)
  if (!libRelative) throw new Error(`Coordenada Maven de Forge invalida: ${forgeLib.name}`)
  const versionRoot = path.join(minecraftRoot, 'versions', versionId)
  fs.mkdirSync(versionRoot, { recursive: true })
  await fs.promises.writeFile(path.join(versionRoot, versionId + '.json'), JSON.stringify(versionJson, null, 2))
  const libDest = path.join(minecraftRoot, 'libraries', ...libRelative.split('/'))
  fs.mkdirSync(path.dirname(libDest), { recursive: true })
  await fs.promises.writeFile(libDest, universalBuf)
  logOnly('debug', `Forge legacy instalado nativo: ${versionId}`)
  return versionId
}

async function installForgeWithOfficialInstaller(instance, minecraftRoot, javaPath) {
  const mcVersion = String(instance.version || '').trim()
  const { core: forgeVersion, versions: artifactCandidates } = getForgeInstallerCandidates(mcVersion, instance.loaderVersion)
  if (!mcVersion || !forgeVersion || !artifactCandidates.length) {
    throw new Error(`Versión de Forge inválida para Minecraft ${mcVersion || 'desconocido'}.`)
  }
  ...
  if (!installerPath) {
    ...
    if (!downloaded) {
      ...
      const tried = artifactCandidates.join(' / ')
      // Caso tipico: pack de PvP que pide Forge de 1.8.9 sobre Minecraft 1.8.
      // Forge 11.15.x no existe para 1.8 (ahi lo ultimo es 11.14.4.1577).
      const hint = (mcVersion === '1.8' && /^11\.15\./.test(forgeVersion))
        ? ` Forge ${forgeVersion} es de Minecraft 1.8.9, no existe para Minecraft 1.8 (ahi lo ultimo es 11.14.4.1577): cambia la instancia a 1.8.9 o usa Forge 11.14.4.1577.`
        : ' Verifica que esa combinacion exista en files.minecraftforge.net.'
      throw new Error(`No se encontro el instalador de Forge ${forgeVersion} para Minecraft ${mcVersion} (HTTP 404, se probo: ${tried}).${hint}`)
    }
  }
  ...
  if (isLegacyInstaller) {
    ...
    const xmclCore = await getXmclCore()
    const resolvedVersion = await xmclCore.Version.parse(minecraftRoot, legacyVersionId)
    if (!await hasValidXmclGeneratedArtifacts(minecraftRoot, resolvedVersion)) {
      throw new Error(`Forge termino sin generar todos los artefactos requeridos (${legacyVersionId}).`)
    }
  ...
  const { spawn } = require('child_process')
  const FORGE_INSTALL_TIMEOUT_MS = 10 * 60 * 1000
  let outputTail = ''
  const appendOutput = (data) => {
    outputTail = (outputTail + data.toString()).slice(-65536)
  }
  const persistForgeLog = async () => {
    try {
      await fs.promises.writeFile(path.join(minecraftRoot, 'forge-installer.log'), outputTail)
    } catch {}
  }
  const exitCode = await new Promise((resolve, reject) => {
    ...
    const cleanup = (child) => {
      ...
      try { child.stdout.removeListener('data', appendOutput) } catch {}
      try { child.stderr.removeListener('data', appendOutput) } catch {}
    ...
    child.once('close', (code, signal) => {
      ...
      settled = true
      cleanup(child)
      if (signal || (inicioCancelado || xmclCancellationRequested)) {
        reject(new Error('Instalación de Forge cancelada.'))
        return
      }
      if (code === null) {
        reject(new Error(`El instalador oficial de Forge no terminó (timeout ${Math.round(FORGE_INSTALL_TIMEOUT_MS / 60000)}min).`))
        return
      }
      resolve(code)
    })
  }).catch(async (error) => {
    await persistForgeLog()
    throw error
  })
  if (exitCode !== 0) {
    const usefulTail = outputTail.split(/\r?\n/).filter(Boolean).slice(-20).join('\n')
    // Guardar salida completa para diagnostico (la consola solo muestra un resumen).
    await persistForgeLog()
    throw new Error(`El instalador oficial de Forge termino con codigo ${exitCode} (ver forge-installer.log en la instancia).\n${usefulTail}`)
  }
  await persistForgeLog()

  let launcherProfiles = null
  try {
    launcherProfiles = JSON.parse(await fs.promises.readFile(launcherProfilesPath, 'utf8'))
  } catch (error) {
    try {
      await fs.promises.copyFile(launcherProfilesPath, `${launcherProfilesPath}.corrupt-${Date.now()}.bak`)
    } catch {}
    throw new Error(`launcher_profiles.json corrupto tras instalar Forge (${instance.version}-forge-${forgeVersion}): se guardó backup .corrupt-*.bak. Reintentá reparar la instalación.`)
  }
  ...
  try {
    ...
    if (minecraftProcess && minecraftProcess.on) {
      ...
      minecraftProcess.on('close', (code, signal) => {
        ...
        currentLogFile = null
        try {
          if (discordRPC) discordRPC.notifyDiscordIdle()
        } catch {}
      })
    }
    ...
    try {
      if (discordRPC) discordRPC.notifyDiscordPlaying(instance)
    } catch {}
    return { ok: true }
...
ipcMain.handle('discord-set-enabled', async (_event, enabled) => {
  try {
    const settings = loadLauncherSettings()
    settings.discordEnabled = Boolean(enabled)
    saveLauncherSettings(settings)
    if (discordRPC) {
      const result = await discordRPC.setDiscordEnabled(settings.discordEnabled)
      if (settings.discordEnabled) {
        const running = getInstance(runningInstanceId || activeLaunchInstanceId)
        if (running && (minecraftProcess || launchRequestInProgress)) {
          await discordRPC.notifyDiscordPlaying(running).catch(() => {})
        } else {
          await discordRPC.notifyDiscordIdle().catch(() => {})
        }
      }
      return result
    }
    return { ok: true, enabled: settings.discordEnabled }
  } catch (error) {
    return { ok: false, error: error?.message || String(error) }
  }
})

ipcMain.handle('discord-get-state', () => {
  try {
    const settings = loadLauncherSettings()
    if (discordRPC && typeof discordRPC.getDiscordState === 'function') {
      const state = discordRPC.getDiscordState()
      return { ...state, enabled: settings.discordEnabled !== false }
    }
    return { ok: true, enabled: settings.discordEnabled !== false, connected: false, configured: false }
  } catch (error) {
    return { ok: false, error: error?.message || String(error) }
  }
})
...
ipcMain.handle('discord-set-activity', async (_event, payload = {}) => {
  try {
    if (!discordRPC) return { ok: false, reason: 'unavailable' }
    const settings = loadLauncherSettings()
    if (settings.discordEnabled === false) return { ok: false, reason: 'disabled' }
    return await discordRPC.setDiscordState(payload.state || 'idle', payload.info || {})
  } catch (error) {
    return { ok: false, error: error?.message || String(error) }
  }
})
